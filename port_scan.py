...
#по умолчанию сканирует по большому списку
from scapy.all import *
import argparse
from colorama import Fore

ports_big = [20, 21, 22, 23, 25, 42, 43, 53, 67, 69, 80, 110, 115, 123, 137, 138, 139, 143, 161, 179, 443, 445, 514, 515, 993, 995, 1080, 1194, 1433, 1702, 1723, 3128, 3268, 3306, 3389, 5432, 5060, 5900, 5938, 8080, 10000, 10050, 20000]
ports_small=[25,80,53,443,445,3389, 8080,8443]
...
def get_arguments():
    ...
    parser.add_argument('ip_addr',type=str,help="enter ")
...
def scan_m(ip,iface):
    ...
    if ans:
        ...
        for (s,r,) in ans:
            a,u = sr(IP(dst=r[ARP].psrc)/TCP(sport=33333,dport=ports_big,flags="S"),iface=iface,timeout=2,verbose=False)
            if a:
                open_ports=[]
                for (s1,r1) in a:
...
def main():
    args = get_arguments()
    try:
        scan_m(args.ip_addr,args.iface)
        print('[+] stop scaning.\n')
    except PermissionError:
        print('\n\r[-] sorry...u need sudo privilege...')
    except KeyboardInterrupt:
        print('\n\r[-] Detected CTRL+C. Stop scaning')
main()
